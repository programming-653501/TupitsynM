...
void __fastcall TLoginForm::LoginButtonClick(TObject *Sender)
{
	...
	if (!CurrentUser)
	{
		...
		else
		{
            switch (rand() % 5)
			{
				case 0: ShowMessage("Добро пожаловать, " + CurrentUser->GetName() +"! Удачной тренировки!");;
						break;
				case 1: ShowMessage("Ого," + CurrentUser->GetName() + ", давно не виделись!");
						break;
				case 2: ShowMessage(CurrentUser->GetName() + ", рад Вас видеть!");
						break;
				case 3: ShowMessage("Ура! " + CurrentUser->GetName() + "! Я уже боялся Вас больше никогда не увидеть!");
						break;
				case 4: ShowMessage("Кто бы мог подумать! " + CurrentUser->GetName() + "! Удачи в сегодняшней тренировке!");
						break;
			}
			PasswordEdit->Clear();
			LoginForm->Hide();
